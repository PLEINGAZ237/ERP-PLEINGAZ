...
export default function ValidationsEntrees() {
  ...
  const load = async () => {
    setLoading(true)
    // Mouvements d'entrée du jour sur les centres enfûteurs
    ...
    const { data: v } = await supabase.from('validations_flux')
      .select('*').eq('type', 'entree').order('created_at', { ascending: false }).limit(50)
    setValidations(v ?? [])
    setLoading(false)
  }
  ...
  const validerEntree = async (mvt) => {
    ...
    const { error: err } = await supabase.from('validations_flux').insert({
      type: 'entree', magasin_id: mvt.journees_stock?.magasin_id,
    ...
    setSuccess(qte === mvt.quantite ? 'Entrée validée — quantité conforme.' : `Écart détecté : compté ${qte} au lieu de ${mvt.quantite}.`)
    load()
  }
  ...
  return (
    <ControleurFluxLayout>
      ...
      <p className="text-sm text-gray-400 mb-6">Comptez et validez les entrées avant le magasinier (centre enfûteur).</p>
      ...
      {mouvements.length === 0 ? <p className="text-gray-400 text-sm text-center py-12">Aucune entrée à valider.</p> : (
            ...
            <div key={m.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              {!dejaValide(m.id) && (
                <div className="flex items-center gap-3 mt-3">
                  ...
                  <button onClick={() => validerEntree(m)} disabled={saving === m.id}
