...
export default function ValidationsEntrees() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from('mouvements_stock')
      .select('*, articles(id, nom), journees_stock(magasin_id, magasins(nom, est_centre_enfuteur))').eq('type', 'sortie')
      .order('created_at', { ascending: false }).limit(50)
    setMouvements((data ?? []).filter(m => m.journees_stock?.magasins?.est_centre_enfuteur))
    const { data: v } = await supabase.from('validations_flux').select('*').eq('type', 'sortie').order('created_at', { ascending: false }).limit(50)
    setValidations(v ?? [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
  ...
  const validerEntree = async (mvt) => {
    ...
    const { data, error: err } = await supabase.rpc('valider_flux_sortie', {
      p_magasin_id: mvt.journees_stock?.magasin_id, p_article_id: mvt.articles?.id,
      p_quantite_declaree: mvt.quantite, p_quantite_comptee: qte, p_bon_livraison_id: mvt.bon_livraison_id,
    })
    setSaving(null)
    if (err) { setError(err.message); return }
    if (data?.ecart) setError(`Écart détecté (déclaré ${mvt.quantite}, compté ${qte}). Bloqué — DG et audit notifiés.`)
    else setSuccess('Sortie validée — conforme.')
    load()
  }
  ...
  return (
    <ControleurFluxLayout>
      ...
      <p className="text-sm text-gray-400 mb-6">Comptez et validez les sorties après le magasinier.</p>
      ...
      {mouvements.length === 0 ? <p className="text-gray-400 text-sm text-center py-12">Aucune sortie.</p> : (
