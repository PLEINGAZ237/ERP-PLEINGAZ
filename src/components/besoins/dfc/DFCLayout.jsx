...
const NAV = [
  { path: '/besoins/dfc', label: 'Tableau de bord', icon: LayoutDashboard, exact: true },
  { path: '/besoins/dfc/analyse', label: 'Analyse besoins', icon: BarChart3 },
]

export default function DFCLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-100">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-emerald-900 text-white flex flex-col transform transition-transform duration-300 ease-in-out
      ...
      `}>
        <div className="p-6 border-b border-emerald-800 hidden md:block">
          ...
          <p className="text-xs text-emerald-300 uppercase tracking-widest mt-1">Espace DFC</p>
        </div>
        ...
        <nav className="flex-1 py-4 px-3 space-y-1">
          {NAV.map(({ path, label, icon: Icon, exact }) => {
            const active = exact
              ...
              : location.pathname.startsWith(path) && path !== '/besoins/dfc'
            return (
              <Link
                key={path}
                to={path}
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all ${
                  active ? 'bg-emerald-600 text-white shadow-lg' : 'text-emerald-200 hover:bg-emerald-800'
                }`}
              >
                <Icon size={18} />
                {label}
        ...
        <div className="p-4 border-t border-emerald-800 bg-emerald-950/50">
          <div className="mb-4 px-2">
            <p className="text-sm font-medium text-white truncate">{profile?.prenom} {profile?.nom}</p>
            <p className="text-[10px] text-emerald-400 uppercase font-bold truncate">DFC</p>
          </div>
          <button
